...
def serialize_json(obj, step="", new_step=""):
    global f_found
    if obj is None:
        return "null"
    elif obj is True:
...
def deserialize_json(string, idx):
    if string[idx] == '"':
...
def parse_dict(string, idx):
    ...
    while True:
        if next_char == "}":
            ...
            obj, idx = deserialize_json(string, idx)
            args[temp] = obj
            comma = True
        else:
            raise TypeError(idx)
...
def dict_to_obj(obj):
    try:
        ...
        raise TypeError("Incorrect object")
...
def dumps_list(obj, step="", new_step=""):
    ...
    for i in range(len(obj) - 1):
        res += (
                step
                + serialize_json(obj[i], step, new_step.replace("\n", "") + step)
                + ","
                + new_step
        )
    res += (
            step + serialize_json(obj[-1], step, new_step.replace("\n", "") + step) + new_step + "]"
    )
    return res
...
def dumps_dict(obj, step="", new_step=""):
    ...
    res += (
            ...
            + '"'
            + ": "
            + serialize_json(obj[keys[-1]], step, new_step.replace("\n", "") + step)
            + new_step
            + "}"
    )
    return res
